...
$DBLIB->pageLimit = (isset($_POST['pageLimit']) ? min(100, max(1, intval($_POST['pageLimit']))) : 20); //Users per page
if (isset($_POST['category'])) $DBLIB->where("assetTypes.assetCategories_id", intval($_POST['category']));
if (isset($_POST['manufacturer'])) $DBLIB->where("manufacturers.manufacturers_id", intval($_POST['manufacturer']));
if (isset($_POST['assetTypes_id'])) $DBLIB->where("assetTypes.assetTypes_id", intval($_POST['assetTypes_id']));
$DBLIB->orderBy("assetCategories.assetCategories_id", "ASC");
$DBLIB->orderBy("assetTypes.assetTypes_name", "ASC");
$DBLIB->join("manufacturers", "manufacturers.manufacturers_id=assetTypes.manufacturers_id", "LEFT");
$linkedFilter = !isset($_POST['all']) ? ' AND assets.assets_linkedTo IS NULL' : '';
$DBLIB->where("((SELECT COUNT(*) FROM assets WHERE assetTypes.assetTypes_id=assets.assetTypes_id AND assets.instances_id = ? AND (assets.assets_endDate IS NULL OR assets.assets_endDate >= CURRENT_TIMESTAMP()) AND assets_deleted = 0" . $linkedFilter . ") > 0)", [$AUTH->data['instance']['instances_id']]);
$DBLIB->join("assetCategories", "assetCategories.assetCategories_id=assetTypes.assetCategories_id", "LEFT");
$DBLIB->join("assetCategoriesGroups", "assetCategoriesGroups.assetCategoriesGroups_id=assetCategories.assetCategoriesGroups_id", "LEFT");
if (strlen($PAGEDATA['search']) > 0) {
    //Search
    $searchTerm = '%' . $DBLIB->escape(trim($PAGEDATA['search'])) . '%';
    $DBLIB->where("(
