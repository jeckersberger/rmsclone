...
if ($AUTH->data['instance']["instancePositions_id"]) $DBLIB->where("(cmsPages_visibleToGroups IS NULL OR (FIND_IN_SET(?, cmsPages_visibleToGroups) > 0))", [(int)$AUTH->data['instance']["instancePositions_id"]]); //If the user doesn't have a position - they're server admins
