...
if (!$AUTH->instancePermissionCheck("MAINTENANCE:VIEW")) die($TWIG->render('404.twig', $PAGEDATA));

// Initialize MaintenanceService
$maintenanceService = new MaintenanceService($DBLIB);

$PAGEDATA['pageConfig'] = ["TITLE" => "Wartungs- & Predictive Maintenance", "BREADCRUMB" => false];

// Support legacy MAINTENANCE_JOBS:VIEW permission or new MAINTENANCE:VIEW
if (!$AUTH->instancePermissionCheck("MAINTENANCE_JOBS:VIEW") && !$AUTH->instancePermissionCheck("MAINTENANCE:VIEW")) {
    die($TWIG->render('404.twig', $PAGEDATA));
}

// Check if using legacy view or new dashboard
$view = isset($_GET['view']) ? $_GET['view'] : 'dashboard';

if ($view === 'legacy') {
    // Legacy maintenance view (existing functionality)
    ...
    // New dashboard view with schedules and predictive maintenance
    echo $TWIG->render('maintenance/maintenance_dashboard.twig', $PAGEDATA);
}
?>
